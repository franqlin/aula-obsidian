...
var ALLOW_FILE_EXTENSIONS = ["png", "jpg", "jpeg", "gif", "pdf"];
...
var downloadFiles = async (data, files, resorceFolderName, adapter, useCache = true) => {
  var _a, _b;
...
var saveFiles = async (app, plugin, settings, selectFiles = [], useCache = true) => {
  const notices = [];
  notices.push(new import_obsidian.Notice(START_MESSAGE, 2e3));
  console.log(START_MESSAGE);
  notices.push(new import_obsidian.Notice(PROCESS_MESSAGE, 2e3));
...
var MediaSyncPlugin = class extends import_obsidian2.Plugin {
  ...
  async loadSettings() {
    ...
    try {
      ...
      if (loadData.setting) {
        if (!((_a = this.settings) == null ? void 0 : _a.setting)) {
...
var MediaSyncSettingTab = class extends import_obsidian2.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian2.Setting(containerEl).setName("Folder to store images").setDesc("Specify a folder to store media files.").addDropdown((dropdown) => {
    ...
    const customFolderSetting = new import_obsidian2.Setting(containerEl).setName("Custom folder name").setDesc("Specify folder name  where the media files will be stored.").addText((text) => {
